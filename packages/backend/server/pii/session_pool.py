"""Per-session codec pool — port of ``proxy/src/session.ts``.
...
class ProxySessionPool:
    """Caches one :class:`VaultTokenCodec` per session id.
    ...
    __slots__ = ("_codecs", "_detect", "_restore_options", "_vault")
    ...
    def __init__(
        ...
        *,
        ...
        self._codecs: dict[str, VaultTokenCodec] = {}

    def get(self, headers: Mapping[str, str]) -> VaultTokenCodec:
        session_id = read_session_header(headers) or DEFAULT_SESSION_ID
        codec = self._codecs.get(session_id)
        if codec is None:
